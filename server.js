...
app.get("/player",async(req,res)=>{
    let storeData = await player.find();
    res.json(storeData);
})
  ...
  let playerSchema = new mongoose.Schema({
   ...
   let saveDataIntoDB = async ()=>{
    ...
    let getDataFromDB = async()=>{
        let storedDataFromDB = await Player.find();
...
let connectToMongoDB = async()=>{
    ...
    try{
        await mongoose.connect("mongodb+srv://srinivas:srinivasg@srinivas123.tvzse0v.mongodb.net/?retryWrites=true&w=majority&appName=srinivas123");
